...
> **Purpose**: Your lifetime technical reference. Revision at any career level.
...
### [Section 1: Spring Boot & Java Internals](./01_Advanced_SpringBoot_Java_Internals.md) `MID+`

- **1.1 Java Memory Model**: Heap vs. Stack, Escape Analysis, Generational GC
...
- **1.2 Concurrency**: CompletableFuture, Custom Thread Pools, Race Conditions
...
- **1.3 Spring Magic**: @SpringBootApplication, AutoConfiguration, Bean Lifecycle
...
- **1.4 Security**: OAuth2 Flows, JWT, Stateless vs. Stateful
...
### [Section 2: Microservices & Distributed Systems](./02_Microservices_Distributed_Systems.md) `SR+`

- **2.1 Resilience Patterns**: Circuit Breaker, Bulkhead, Retry, Rate Limiting
  - Circuit Breaker state machine (CLOSED → OPEN → HALF_OPEN)
  - Resilience4j configuration with Spring Boot (full YAML + code)
  - Exponential backoff with jitter for retries
  - Semaphore vs. Thread-pool bulkheads
- **2.2 Data Consistency**: SAGA, 2PC, Eventual Consistency
...
- **2.3 Communication**: REST vs. gRPC, Kafka, RabbitMQ
...
### [Section 3: Database & Performance Tuning](./03_Database_Performance_Tuning.md) `MID+`

- **3.1 SQL Optimization**: Indexing, EXPLAIN ANALYZE, N+1 Problem
...
- **3.2 Scaling**: Sharding vs. Partitioning, Read Replicas
...
- **3.3 Caching**: Redis/Hazelcast, Cache-Aside, Write-Through, Eviction
...
### [Section 4: System Design Concepts](./04_System_Design.md) `SR+`

- **4.1 Scalability Concepts**: Vertical vs. Horizontal Scaling, CAP, Load Balancers
...
- **4.2 Design Patterns**: API Gateway, BFF, Strangler Fig
...
### [Section 5: Modern Trends (2026 Focus)](./05_Modern_Trends_2026.md) `LEAD`

- **5.1 AI Integration**: RAG with Spring Boot & Vector Databases
...
- **5.2 Kubernetes for Leads**: Pods, Services, Ingress, Rolling Updates
...
### [Section 6: Angular & Frontend Engineering](./06_Angular_Frontend_Engineering.md) `ALL`

- **6.1 Angular Architecture (Angular 17+)**: Standalone Components, Signals, New Control Flow
...
- **6.2 State Management**: Service-based Signals vs. NgRx Signal Store
...
- **6.3 RxJS Mastery**: The Four Map Operators, Combination Operators, Memory Leaks
...
- **6.5 HTTP Client, Interceptors, and Error Handling**
...
- **6.6 Performance Optimization**: OnPush, Virtual Scrolling, Bundle Analysis
...
- **7.2 Records**: Immutable data carriers, pattern matching
- **7.3 Sealed Classes**: Exhaustive type hierarchies, domain modeling
- **7.4 Pattern Matching**: instanceof, switch expressions, guarded patterns
- **7.5 Text Blocks**: Multi-line strings for SQL, JSON, HTML templates

### [Section 8: Testing Strategies](./08_Testing_Strategies.md) `MID+`

- **8.1 Test Pyramid**: Unit → Integration → E2E ratios and trade-offs
- **8.2 Unit Testing**: JUnit 5, Mockito, parameterized tests, BDD-style
- **8.3 Integration Testing**: Spring Boot Test, Testcontainers, real DB/Kafka
- **8.4 Contract Testing**: Spring Cloud Contract, consumer-driven contracts
- **8.5 Performance Testing**: JMeter, Gatling, load profiles, SLA validation
- **8.6 Security Testing**: OWASP ZAP, SonarQube, dependency scanning
- **8.7 E2E Testing**: Playwright, Cypress, Page Object Model

### [Section 9: DevOps, CI/CD, Docker & Containerization](./09_DevOps_CICD_Docker.md) `SR+`

- **9.1 Docker**: Multi-stage builds for Java/Angular, layer caching, distroless images
  - Production Dockerfiles with security best practices
  - Docker Compose for local development environments
- **9.2 CI/CD Pipelines**: GitHub Actions & GitLab CI complete examples
  - Pipeline stages: build → test → analyze → package → deploy
  - Blue-Green, Canary, and Rolling Update deployment strategies
  - Trunk-based development vs. GitFlow
- **9.3 GitOps**: ArgoCD, pull-based deployments, drift detection
...
- **9.5 Infrastructure as Code**: Terraform for AWS (EKS, RDS, ElastiCache)
- **9.6 DevSecOps**: SAST/DAST/SCA, container scanning, secrets management

### [Section 10: Observability — Logging, Metrics & Distributed Tracing](./10_Observability.md) `SR+`

- **10.1 Three Pillars**: How logs, metrics, and traces work together
- **10.2 Structured Logging**: JSON logging, MDC, ELK Stack vs. Grafana Loki
  - Logback configuration for Spring Boot (dev vs. production)
  - Log aggregation architecture
- **10.3 Metrics**: Prometheus, Grafana, Micrometer, RED & USE methods
  - Custom business metrics, histogram buckets, SLI/SLO/SLA framework
  - Alerting rules and Alertmanager configuration
- **10.4 Distributed Tracing**: OpenTelemetry, Jaeger, context propagation
  - Auto-instrumentation and custom spans
  - Sampling strategies (head-based, tail-based, parent-based)
- **10.5 Grafana Dashboards**: Four essential dashboards for Lead Engineers
- **10.6 Alerting Strategy**: Symptom-based alerting, error budgets, runbooks

### [Section 11: API Design & REST Best Practices](./11_API_Design_REST.md) `MID+`

- **11.1 RESTful Design**: Resource-oriented URLs, HTTP methods, idempotency
  - Idempotency key implementation for safe retries
  - HTTP status codes cheat sheet
- **11.2 Error Handling**: RFC 7807 Problem Details, global exception handler
- **11.3 Pagination**: Cursor-based vs. offset-based, filtering, sorting
  - Performance comparison and implementation
- **11.4 API Versioning**: URL path vs. header, breaking vs. non-breaking changes
  - Deprecation strategy with Sunset headers
- **11.5 HATEOAS**: Self-describing APIs with hypermedia links
- **11.6 OpenAPI/Swagger**: Auto-generated documentation from code
- **11.7 API Security**: OAuth2, CORS, rate limiting, input validation

### [Section 12: Design Patterns, SOLID & Clean Architecture](./12_Design_Patterns_SOLID_CleanArch.md) `MID+`

- **12.1 SOLID Principles**: Real-world examples with violations and fixes
  - SRP with Spring Events, OCP with Strategy pattern
  - LSP, ISP with segregated interfaces, DIP with Spring DI
- **12.2 GoF Patterns**: Factory, Builder, Decorator, Adapter, Strategy, Observer, Template Method
  - Each pattern with Spring Boot real-world examples
- **12.3 Microservices Patterns**: CQRS, Event Sourcing
  - CQRS implementation with separate read/write models
  - Event Sourcing with domain events and aggregate replay
- **12.4 Clean Architecture**: Hexagonal/Ports & Adapters
  - Full package structure with domain, application, adapter layers
  - Dependency Rule — inner layers never depend on outer layers
- **12.5 Domain-Driven Design**: Bounded Contexts, Aggregates, Value Objects

### [Section 13: Leadership, Behavioral & System Design Interviews](./13_Leadership_Behavioral_SystemDesign.md) `LEAD`

- **13.1 STAR Method**: Structuring behavioral answers with concrete examples
  - Technical leadership stories
  - Mentoring and conflict resolution stories
  - Delivery under pressure and project failure stories
- **13.2 Leadership Principles**: Ownership, Bias for Action, Earn Trust
  - Preparing your story bank (10 stories covering all categories)
- **13.3 System Design Framework**: 4-step approach for 45-minute interviews
...
- **13.4 Common System Design Questions**: Key components and trade-offs
- **13.5 Architecture Decision Records (ADRs)**: Documenting technical decisions
- **13.6 Estimation & Planning**: PERT estimation, technical roadmapping

### [Section 14: Application Security, OWASP & Cloud Fundamentals](./14_Security_OWASP_Cloud.md) `SR+`

- **14.1 OWASP Top 10 (2021)**: Complete guide with Spring Boot prevention code
...
- **14.2 JWT Security Deep Dive**: Algorithm confusion, token storage, RS256, refresh rotation
- **14.3 CORS**: Same-Origin Policy, preflight, Spring Boot configuration
- **14.4 Cloud Fundamentals for Backend Engineers**: AWS core services, deployment architecture
...
- **14.5 Security in System Design Interviews**: 8-point security checklist
- **14.6 Interview Quick Reference**: OWASP one-liners, security headers table
